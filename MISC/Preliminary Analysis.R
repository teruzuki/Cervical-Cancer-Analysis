...
summary(glm(Cancer ~ numSexPart, family = "binomial"))
summary(glm(Cancer ~ yearPills, family = "binomial"))
summary(glm(Cancer ~ yearSmoke, family = "binomial"))
summary(glm(Cancer ~ STDhpv, family = "binomial"))
#linear model of HPV
lmHPVc <- (glm(Cancer ~ STDhpv, family = "binomial"))
#ggplot(lmHPVc, )

correlationAll <- round(cor(MasterFile), 1)
head(correlationAll[, 1:6])
install.packages("corrplot")
library(corrplot)
corrplot(correlationAll)
#correlationSTDs <- round(cor(STDSuiteCleaned), 1)

ggcorrplot(STDSuiteCleaned)
